...
class FedLedger:
    ...
    def submit_validation_result_orbis(self, assignment_id: int, loss: float):
        orbis_url = "http://localhost:8080/write"
        print(assignment_id, 'assignment_id')
        orbisResponse = requests.post(
            orbis_url,
            headers=self.headers,
            json={
                "loss": loss,
        ...
        print(orbisResponse.text)
        return orbisResponse
